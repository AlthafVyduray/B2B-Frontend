...
const useAdminStore = create((set, get) => ({
  ...
  getBookings: async ({ stateFilter = "", searchTerm = "", page = 1, limit = 10 } = {}) => {
    ...
    try {
      ...
      set({
        ...
        bookingPagination: res.data.pagination || { page, totalPages: 1, limit, total: 0 },
        loadBookings: false
      });
    } catch (error) {
    ...
    confirmBooking: async (id) => {
        try {
            const res = await axiosInstance.put(`/admin/booking-details/confirm/${id}`);
           set((state) => ({
                bookings: state.bookings.map(booking =>
    ...
    createUser: async ({ email, password, role }) => {
        try {
            const res = await axiosInstance.post('/admin/users', { email, password, role });
            await get().getUsers();
