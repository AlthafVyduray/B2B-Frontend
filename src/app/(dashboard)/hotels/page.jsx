...
export default function HotelsPage() {
  ...
  const handleFileChange = (e) => {
    ...
    if (!allowed.includes(file.type)) {
      setErrors((p) => ({ ...p, image: "Only JPG / PNG / WEBP images allowed" }));
      return;
    }
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      if (formData.starRating !== "") fd.append("starRating", String(formData.starRating));

      if (formData.pricing?.cpPrice !== "") fd.append("cpPrice", String(formData.pricing.cpPrice));
      if (formData.pricing?.apPrice !== "") fd.append("apPrice", String(formData.pricing.apPrice));
      if (formData.pricing?.mapPrice !== "") fd.append("mapPrice", String(formData.pricing.mapPrice));
      if (formData.pricing?.roomPrice !== "") fd.append("roomPrice", String(formData.pricing.roomPrice));
      if (formData.pricing?.extraBedPrice !== "") fd.append("extraBedPrice", String(formData.pricing.extraBedPrice));
      ...
      const msg = err?.response?.data?.message || err?.message || "Failed to create hotel";
      alert(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      alert(err?.response?.data?.message || err?.message || "Failed to update hotel");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="bg-white flex min-h-screen lg:mt-0 mt-10 m-2">
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-2/3 p-6 space-y-3">
                  ...
                  <div className="text-sm text-gray-700">
                    <span className="font-medium">Room:</span> ₹{hotel.pricing.roomPrice}
                    <span className="ml-4 font-medium">Extra Bed:</span> ₹{hotel.pricing.extraBedPrice}
                    <span className="ml-4 text-gray-500">CP ₹{hotel.pricing.cpPrice}, AP ₹{hotel.pricing.apPrice}, MAP₹{hotel.pricing.mapPrice}</span>
                  </div>

                  <div className="flex gap-2 pt-2">
                    <Button onClick={() => setHotelToUpdate(hotel)} variant="outline" size="sm" className="flex-1">
                      <Edit className="w-4 h-4 mr-1" /> Edit
                    </Button>
                    <Button onClick={() => setHotelToDelete(hotel)} variant="outline" size="sm" className="text-red-600 flex-1">
      ...
      {hotelToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setHotelToDelete(null)} />
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-auto overflow-hidden z-10">
            <div className="flex items-start justify-between p-5 border-b">
              ...
              <button onClick={() => setHotelToDelete(null)} aria-label="Close" className="p-1 text-gray-500 hover:text-red-500">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6">
              <p className="text-sm text-gray-700 mb-4">
                Are you sure you want to delete <strong className="text-gray-900">{hotelToDelete?.name ?? "this hotel"}</strong>? This action cannot be undone.
              </p>

              <div className="text-sm text-gray-500 mb-4">
                <div>ID: {hotelToDelete?._id ?? "—"}</div>
                <div>Created: {hotelToDelete?.createdAt ? String(hotelToDelete.createdAt).slice(0, 10) : "—"}</div>
              </div>
      ...
      {hotelToCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-black/25 backdrop-blur-sm" onClick={() => setHotelToCreate(false)} />
          <div className="relative z-10 p-6 bg-white rounded-lg shadow-xl w-full max-w-3xl mx-auto max-h-[90vh] overflow-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setHotelToCreate(false)} className="text-gray-500 hover:text-gray-700"><X size={20} /></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className="flex flex-col">
                  ...
                  <input name="name" value={formData.name} onChange={handleChange} className="border rounded px-3 py-2" placeholder="Hotel name" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className="flex flex-col">
                  ...
                  <p className="text-xs text-gray-500 mt-1">Optional. Max 5 MB. JPG / PNG / WEBP.</p>
                </label>
              </div>
              ...
              {imagePreview && (
                <div className="w-48 p-2 border rounded">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button type="button" onClick={() => { if (imagePreview.startsWith("blob:")) URL.revokeObjectURL(imagePreview); setImagePreview(""); setImageFile(null); }} className="text-gray-500 hover:text-gray-700">
                      <X size={16} />
                    </button>
                  </div>
                  <img src={imagePreview} alt="preview" className="w-full h-32 object-cover rounded" />
              ...
              <div className="flex justify-end gap-3 pt-4 border-t">
                ...
                <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700" disabled={submitting}>
